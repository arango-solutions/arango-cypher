...
### Bug-fix wave — Schema inference & NL feedback loop (WP-27..WP-30) — **Shipped**
...
**Status (2026-05-06): all four WPs are merged to `main`.** Phase A/B (WP-27 + WP-28) shipped 2026-04-23 across commits `0313395` (heuristic + transpiler) and `ac6eaff` (analyzer visibility + force-reacquire + UI banner). Phase C (WP-29 + WP-30) shipped 2026-04-24 across commits `d4a871f` (NL prompt escaping + fail-closed retry) and `618aace` (`editorCypherSource` reducer + regenerate-from-NL action). The remaining open item is the §11.4 closeout E2E walkthrough on `ic-knowledge-graph-temporal` (manual; expected when an operator can connect to the pilot DB) — until that runs, the bug-fix PRD remains marked OPEN even though the implementation is complete. Sub-sections below are kept verbatim for historical reference.
...
WP-27 and WP-28 were independent and landed together as Phase A/B (library + service, single PR). WP-29 and WP-30 landed as Phase C (separate PR; UI ship) after A/B validation.
...
## WP-NG: Optional named-graph scoping

**PRD**: §17 (Named-graph scoping)
**Priority**: User-requested feature; subsumes the "schema hygiene" follow-up
**Estimate**: ~3-4 days (backend + UI + tests)

### Scope

Let an operator optionally restrict a session to a single ArangoDB **named
graph** so schema introspection, the NL prompt context, transpilation, and
execution consider only that graph's collections. No graph selected → all
collections (unchanged default).

### Slices (incremental, each independently verifiable)

1. **Acquire-layer scoping.** `graph_collections(db, name)` resolver +
   `_filter_bundle_to_graph(bundle, vertex_colls, edge_colls)`; thread
   `graph_name` through `get_mapping` → `_build_fresh_bundle` →
   `acquire_mapping_bundle`; graph-aware cache keys in both tiers. Tests:
   `tests/test_schema_acquire_graph_scope.py`.
2. **Session field.** `_Session.graph_name` slot.
3. **Endpoints.** `GET /graphs` (list named graphs + collections);
   `POST /session/graph` (bind/clear, validate existence). Wire
   `session.graph_name` into the schema endpoints. Tests:
   `tests/test_service_graphs.py`.
4. **UI.** `GraphSelector` toolbar control populated from `/graphs`, persisted
   per `(url, database)` in `localStorage`, re-introspects on change; default
   "All collections".

### Acceptance criteria

- With no graph bound, `/schema/introspect` output is identical to today.
- With a graph bound, only that graph's vertex/edge collections appear in the
  introspected mapping; the schema cache for the scoped view is isolated from the
  unscoped view.
- `POST /session/graph` with an unknown graph returns 404; with `null` clears the
  binding.
- Full unit suite green; UI typecheck + build green.
...
| WP-27 | Heuristic type-field detection hardening (D1 + D5) | v0.4 | **Done** | Merged 2026-04-23 (commit `0313395`, PR `fix/wp27-heuristic-type-field-detection`). Tier-split `_DOC_TYPE_FIELDS` into `_TIER1_TYPE_FIELDS` (`type`, `_type`, `entityType`) + `_TIER2_TYPE_FIELDS` (`label`, `labels`, `kind`); tier-2 candidates additionally require distinct ≤ `max(50, 0.5·rows)` AND every distinct value passes `_looks_class_like` (no `.`/`/`/whitespace, no file-extension suffix); rejection reasons captured in `metadata.heuristic_notes`. Transpiler `_strip_label_backticks` applied at every `resolver.resolve_entity` / `resolve_relationship` call site in `arango_cypher/_translate_v0/`. Closes D1 + D5 of [`schema_inference_bugfix_prd.md`](./schema_inference_bugfix_prd.md) §4.1, §4.5. |
...
| WP-CAT | Schema catalog sidecar | v0.4 | **Done** | 2026-06-22. PRD §19. Moves schema analysis off the request path: sidecar (`arango_cypher/catalog/` + `scripts/catalog_sync.py`) keeps a registry (`configs/catalog.yml`, env-ref creds) of databases warm in the shared persistent cache; `schema_acquire.read_cached_mapping` serves the catalog read-only (no fingerprint/analyze/threads); `/schema/introspect` + `/schema/statistics` return `status: ready|pending`; UI `introspectSchemaUntilReady` polls on pending. Staleness bounded by `interval_seconds` (default 30 min); "Refresh schema" still forces a rebuild. Tests: `test_catalog_registry.py`, `test_catalog_sync.py`, `read_cached_mapping` + pending-endpoint cases. |
