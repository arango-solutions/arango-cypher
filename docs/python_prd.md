...
- ~~Refresh `tests/nl2cypher/eval/baseline.json` with a real-LLM report~~ Done 2026-04-18 (OpenAI gpt-4o-mini, `full` config, 31 cases, parse_ok=100%, pattern_match=87.1%). Remaining: turn the gate on in nightly CI (currently opt-in via `RUN_NL2CYPHER_EVAL=1`).
