...
Save the `name` and `version` values — you will need them in the deployment step.
...
|----------------------|----------|-----------------------------------------------------------------------------------------------|----------|
| `app_name`           | body     | Must match the `name` from the upload step.                                                   | Yes      |
| `app_version`        | body     | Must match the `version` from the upload step.                                                | Yes      |
| `service_type`       | env      | Set to `base_type`.                                                                           | Yes      |
| `base_image`         | env      | Base image name (see *Available Base Images*).                                                | Yes      |
| `app_instance_name`  | env      | Service instance name (alphanumeric, 32-character length limit, used in routing).             | Yes      |
...
A complete workflow for uploading and deploying a database-scoped service from a code package:
