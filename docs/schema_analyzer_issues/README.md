...
**Status (2026-04-28):** all eight issues are resolved upstream, adopted downstream, and **closed out**. Issues #01–#07 shipped in mapper v0.2.0 / v0.3.0 (Wave 4q, see the [downstream migration note](https://github.com/ArthurKeen/arango-schema-mapper/pull/11)); issue #06 adoption for the NL multi-tenant guardrail landed 2026-04-23 (see the downstream note on `06-emit-tenant-scope-annotations.md`); issue #08 shipped in mapper v0.5.0 / v0.6.0 (PRs #15 / #16 / #17), was adopted locally via `arango-cypher-py` PR #6 (`feat/adopt-sharding-profile`) on 2026-04-23, and was closed out on 2026-04-28 (see the close-out summary at the bottom of [`08-emit-physical-layout-and-shard-topology.md`](./08-emit-physical-layout-and-shard-topology.md)). One upstream follow-up — VCI first-class emission, §6.2 bullet 2 — remains open on the analyzer roadmap; it is orthogonal to multi-tenancy and not blocking any downstream work here.
...
| [09](./09-consume-named-graph-membership.md) | Consume `physicalMapping[*].graphs` + reconstruct `metadata.graphMembership` for named-graph scoping | (none — replaces the live `graph_collections` lookup on the happy path; retained as heuristic-tier fallback) | Unreleased atop 0.8.0 | **consumer adopted (back-compat / prefer-embedded-with-fallback)**; pin bump pending an analyzer release |
