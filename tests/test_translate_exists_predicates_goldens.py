"""Golden tests for WP-10: EXISTS, pattern predicates, and additional built-in functions."""
...
_CASES_FILE = Path(__file__).resolve().parent / "fixtures" / "cases_v03" / "exists_and_predicates.yml"
_CASES = yaml.safe_load(_CASES_FILE.read_text(encoding="utf-8"))


@pytest.mark.parametrize("case", _CASES, ids=lambda c: c["id"])
def test_exists_and_predicates(case: dict) -> None:
